...
function SellerLogin() {
  ...
  async function handleLogin(e) {
    e.preventDefault();
    try {
      const Resp = await axios.post(`${import.meta.env.VITE_SERVER_URL}/user/login`, { email, password })
      localStorage.setItem('token', Resp.data?.token)
      console.log("handleLogin ~ Resp:", Resp.data)
      localStorage.setItem('userData', JSON.stringify(Resp.data?.userData))
      toast.success(Resp?.data?.message);
      return navigate('/seller/dashboard/addproduct');
    } catch (error) {
      ...
      <section className="bg-gray-50">
        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
          <Link to="/" className="flex items-center mb-6 text-2xl font-bold text-gray-900">
            <span className="self-center font-bold text-2xl sm:text-2xl text-[#ebb93f]">
              SpokeBee
            </span>
          </Link>
